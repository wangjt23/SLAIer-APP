...
## 22. 第十八轮：考勤打卡天数按学院口径折算「每周最多 5 天」（2026-09-10）

用户给出的学院口径：

> 一周 7 天，任意 5 天打卡就可以，而不是非得工作日打卡；给的那 3 天是用于补卡的。
> 工作日打 5 小时 30 分钟也会被记为合格（官方说是 6 小时）—— 不是说要把目标改成 5.5，
> 而是说明学校的度量口径和界面上看到的不完全一样。建议：该周学校判定合格数每周最多 5 天，
> 超过 5 天后就不算打卡。

### 22.1 先澄清"谁在算什么"

查完之后发现：**学校判定一直就是学校给的，App 从来没自己判过合格**。

| 显示 | 来源 | 谁算 |
|---|---|---|
| 每日「学校判定合格 / 未达标」 | 每日行 `isQual` | 学校 |
| 「合格 / 不合格」 | `stats.isMonthlyQualified` + `qualificationMessage` | 学校 |
| 「已达标 / 应达标 9 / 20 天」 | `stats.actualWorkdayPunches / requiredPunches` | 学校 |
| 「工作日 20 天」 | `stats.totalWorkdays` | 学校 |
| 「补卡额度 3 次」 | `stats.maxAllowedRestdayPunches`（已用：`actualRestdayPunches`） | 学校 |
| 「今天/某天在馆 X 小时」 | 闸机流水配对（只算教学楼、只进不出次日 05:00 作废） | App |

缺的只有**"周"这一层**：学校接口不给周计数，`weeks[]` 只有 `range` + `label`，
合格与否只挂在每一天上，所以"每周最多 5 天"只能本地对**学校的 `isQual`** 做计数封顶。

### 22.2 用户的说法有数据支持

抓到的 2026-09 `stats`：`totalWorkdays=20`、`requiredPunches=20`，而该月实际有 **22** 个工作日
—— 20 恰好等于 4 周 × 5 天，而不是"每个工作日一天"。另外学校把工作日与周末**分开记**
（`actualWorkdayPunches` / `actualRestdayPunches`），`maxAllowedRestdayPunches=3` 就是补卡额度。
周边界也是周一~周日（`2026-08-31`(一) ~ `2026-09-06`(日)），与日历周一致。

### 22.3 改动

| # | 改动 | 说明 |
|---|---|---|
| 1 | `AttendanceWeek.COUNTED_DAYS_PER_WEEK = 5` | 一周最多计入 5 天；`countedDates` / `overflowDates` / `countedCount` 都是纯函数，可单测 |
| 2 | 明细改为**按周分组**渲染，周标题显示「计入 N/5 天」 | 周分组用学校给的 `weeks[].range`，不自己划周 |
| 3 | 超出的合格日在行内写明「已满 5 天不计入」 | 否则用户会问"学校都说合格了，为什么天数对不上" |
| 4 | 「周末可用」→「补卡额度 · 已用 N 次」 | 原叫法会被理解成"周末打卡受限"；已用次数用学校的 `actualRestdayPunches` |
| 5 | 月度卡的「7 / 20 天」「20 天」改为走字符串资源 | 顺手修掉英文界面下仍显示中文单位的问题 |
| 6 | 周标题用**本地化日期范围**，不用数据层拼的中文 label | 缓存里只有每日行，学校给的「第N周 (…)」没被持久化，原来的 `"9月7日起"` 在英文界面会漏中文，而且和范围重复 |
| 7 | 删掉 `AttendanceUiState.qualifiedCount`（整月不封顶地数合格天数） | 正是会和学院口径对不上的算法，且无人使用 |

**没有动**：6 小时目标、`isQual` 的判定、`stats` 里的任何数字 —— 用户明确说不要改目标值。

### 22.4 验证

模拟器上没有真账号，所以用 **debug 包 + 直接往本地 Room 库塞一天假数据**的方式验渲染
（`run-as com.slai.campus.debug sqlite3 databases/slai-campus.db`），数据刻意构造成用户举的两个例子：

| # | 验证项 | 结果 |
|---|---|---|
| 130 | 全周 7 天都合格 → 只计前 5 天（周一~周五），周末两天标「已满 5 天不计入」 | ✅ 截图 |
| 131 | 周一没来、周二~周五来、周六来补 → 周六算，周日不算 | ✅ 截图（「计入 5/5 天」+ 9/13 标注） |
| 132 | 不足 5 天按实际计（3 天 → 计入 3） | ✅ 单测 |
| 133 | 记录顺序打乱不影响结果（按日期排序取前 5） | ✅ 单测 |
| 134 | 跨月的那一周（8/31 起）照常折算 | ✅ 单测 |
| 135 | 界面能指出"这一天为什么不算"（`isNotCounted`） | ✅ 单测 + 截图 |
| 136 | 月度卡：`3 次 / 补卡额度 · 已用 1 次`；`9 / 20 天 已达标 / 应达标` | ✅ 截图（中英各一次） |
| 137 | 单元测试总数 | ✅ **178 通过 / 0 失败**（新增 6 项） |

### 22.5 仍待确认

- 学校 `isQual` 的**每日阈值**依旧不明（§9.6：5:35 合格、4:29 不合格）。App 继续只展示、不推断。
- 「每周 5 天」目前是**对学校判定做折算**；如果学院之后改口径（例如允许 6 天），
  只需要改 `AttendanceWeek.COUNTED_DAYS_PER_WEEK` 这一个常量。
- 学校是否会在跨月的那一周重复计入同一周，尚未验证（缓存按月存，周分组是按日期重新算的）。

### 22.6 补记：为什么 App 显示 5 天、学院网站显示 7 天（同日用户实测反馈）

用户装的是上一版，截图里 App 月度卡是「5 / 20 天」，而学院网站同一时刻写着「有效打卡 7 天」。
逐行对完当天的数据：

```text
9/3 5:35 合格   9/4 6:49 合格               ← 工作日
9/5 8:19 合格   9/6 6:37 合格               ← 学校标成「法定节假日」
9/7 9:07 合格   9/8 9:57 合格   9/9 9:13 合格 ← 工作日
合计 7 天（工作日 5 + 非工作日 2）
```
...
| 数字 | 含义 |
|---|---|
| 学校 API `actualWorkdayPunches = 5` | **只数工作日**的达标天数（App 旧版显示的就是这个） |
| 学校 API `actualRestdayPunches = 2` | 周末 / 法定节假日的补卡天数 |
| 学院网站「有效打卡 7 天」 | 两者之和 |

所以「5 / 20 天」被当成"总打卡天数"来读时必然显得少算 —— 这是**展示口径问题，不是数据问题**。

**改动**：月度卡第一格改为学校的**有效打卡总天数**（`actualWorkdayPunches + actualRestdayPunches`），
标签写明「有效打卡 · 工作日 5 + 补卡 2」；第二格保留「工作日达标 / 应达标」（学校月度判定的依据）；
第三格按学院网站的说法改为「剩余补打卡机会」（`maxAllowedRestdayPunches`，网站同名数字）。

**同时验证：本轮"每周最多 5 天"的折算与这组真实数据不冲突** ——
第 1 周（8/31–9/6）合格 4 天 → 计入 4；第 2 周（9/7–9/13）合格 3 天 → 计入 3；合计 **7**，
与学院网站的「有效打卡 7 天」完全一致（两周都没到 5 天上限，所以没有任何折算损失）。

| # | 验证项 | 结果 |
|---|---|---|
| 138 | 用这组真实数字渲染：月度卡 `7 天 / 有效打卡 · 工作日 5 + 补卡 2` + `5 / 20 天` + `3 天 剩余补打卡机会` | ✅ 截图 |
| 139 | 周标题 `计入 4/5 天`、`计入 3/5 天`，合计 7 = 学院网站 | ✅ 截图 |

> 仍待确认（需要真机 + 学院网站对照）：`actualRestdayPunches` 是否就是"已用的补卡天数"。
> 本轮只把它用在"工作日 + 补卡 = 有效打卡"这个对得上网站的等式里，没有单独展示"已用 N 次"。

### 22.7 用户纠正：第一格只给总数，「补卡」是两种不同的东西

> 不用把两个都给出来，就跟学院网站保持一致，用显示「工作日 + 补卡」的总数就行。
> 另外这里的补卡应该不是那 3 天的补卡吧？这是两种补卡。

两点都成立，第二条尤其重要：

| 说法 | 实际含义 | 对应字段 |
|---|---|---|
| 周末 / 节假日**照常打卡**（9/5、9/6 那两天） | 非工作日也计入有效天数 | `actualRestdayPunches` |
| 网站上的「剩余补打卡机会 3 天」 | **漏卡后的补录机会**，和上面无关 | `maxAllowedRestdayPunches` |

反证很清楚：该用户周末打了 2 天，那个数字仍是 3，没有变成 1 —— 说明它不会被非工作日打卡消耗。
所以：

- 第一格改为**只显示一个合计**（`有效打卡 7 天`），不再写"工作日 5 + 补卡 2"（会把两种补卡混在一起）；
- 代码注释与 README 里的用词统一改成"周末/节假日照常打卡"，不再叫"补卡"；
- 第三格保留网站的原始措辞「剩余补打卡机会」，并在文档里写明它与前者无关。

### 22.8 定稿：月度卡只留两格

> 本月考勤（学校统计） 不合格 / 7 天 / 20 天 有效打卡 / 应达标 / 3 天 剩余补打卡机会
> 这样应该就行，不需要 5 这个量。

据此把月度卡收敛成两格，并**彻底移除 `actualWorkdayPunches` 的展示**（`AttendanceStats.progress` 已删，
新增 `requiredDays`）：
...
| # | 验证项 | 结果 |
|---|---|---|
| 140 | 用真实数字渲染：`7 / 20 天 有效打卡 / 应达标` + `3 天 剩余补打卡机会`，界面上不再出现 5 | ✅ 模拟器截图 |
| 141 | 单测 | ✅ **178 通过 / 0 失败** |

---

## 23. 第十九轮：闸机延迟提示 + 校外连不上的引导（2026-09-10）

用户反馈的真实场景：

> 考勤系统刷脸进出时，系统可能会有 5 分钟左右的延迟。我刷出去了，然后不在内网（校园网）下就
> 没法更新考勤系统，于是会一直显示"在馆中"、一直计时，直到回到校园网才行 —— 住宿舍无所谓，
> 住校外就很尴尬。诉求：① 考勤页提示学校的记录可能有 5–10 分钟延迟；② 更新不了学生系统时，
> 如果用户再查询，给一个"请在校园网环境下刷新"的提示。

### 23.1 为什么原来只显示「离线」

`AttendanceRepositoryImpl` 把两类完全不同的失败合并成了同一个结果：

```kotlin
is RemoteResult.NetworkUnavailable -> AttendanceRefreshResult.Offline(cached = ...)
```

`RemoteResult.NetworkUnavailable` 的注释写得很清楚，它同时覆盖 **"设备没网"** 和
**"DNS / 连接超时 / TLS 失败"**。于是用户带着 4G 站在校门口，界面说的是「离线，显示缓存」——
既没解释"为什么在馆中还在涨"，也没告诉他该回校园网。

### 23.2 改动

| # | 改动 | 说明 |
|---|---|---|
| 1 | 新增 `AttendanceRefreshResult.Unreachable(reason)` | 有网但连不上学校主机，与 `Offline` 区分开 |
| 2 | 纯函数 `networkFailureResult(hasNetwork, hasCache, reason)` | 判定逻辑抽出来单测；仓库层用运行时的 `networkMonitor.hasNetwork` 决定归类 |
| 3 | 考勤页常驻一行延迟提示 | 「闸机记录通常有几分钟延时，刚出闸时可能仍显示"在馆中"」 |
| 4 | 连不上时显示提示卡 | 「连不上学生考勤系统：网络是通的，但学校服务器没有响应……回到校园网后刷新即可。」 |
| 5 | 同步结果行 | `Unreachable` → 「连不上学生系统（可能是校外网络）」 |

提示卡放在"今天"卡片正下方（在月度卡之前）—— 用户一进来就能看到，不用滚到底部看同步状态小字。

### 23.3 验证

模拟器上用**真实的失败路径**验证，不是伪造状态：把 Android 的
Private DNS 指到一个不存在的域名（`private_dns_specifier no-such-host.invalid`），
于是"设备有网、学校主机解析不了"——正是离开校园网时的表现：

```text
adb logcat: W SlaiCampus: attendance transport failure: UnknownHostException
            D SlaiCampus: attendance fetch 2026-09: network unavailable: UnknownHostException
```

| # | 验证项 | 结果 |
|---|---|---|
| 142 | 延迟提示常驻显示在"今天"卡片下方 | ✅ 模拟器截图 |
| 143 | DNS 坏掉后点刷新 → 出现「连不上学生考勤系统」提示卡 | ✅ 模拟器截图 |
| 144 | 提示卡文案含"回到校园网后刷新" | ✅ 截图 |
| 145 | 缓存数据不受影响（11 天记录照常显示） | ✅ 截图 |
| 146 | `hasNetwork=false` → 仍然是 `Offline`，不误导成"校园网问题" | ✅ 单测 |
| 147 | 其它结果（Success / SessionExpired / SchemaChanged / ServerError / Failed）都不触发该提示 | ✅ 单测 |
| 148 | 单元测试总数 | ✅ **181 通过 / 0 失败**（新增 3 项） |
...
## 2026-09-14：按当前网页纠正月度统计字段与周上限解释

此前把 `maxAllowedRestdayPunches` 显示成“剩余补打卡机会”不正确。当前 `attendList` 页的实际显示代码为：

```javascript
// 有效打卡
stats.totalValidPunches || 0
// 剩余补打卡机会
Math.max(3 - (stats.crossWeekUsedPunches || 0), 0)
```

本次 App 直接读取 `totalValidPunches`，按 `crossWeekUsedPunches` 计算剩余机会；字段缺失时保留未知状态，不再把上限或工作日/节假日数量之和冒充结果。原始 stats 已被本地缓存保留，读取缓存也经过同一解析函数。

网页“出勤达标规则说明”明确：考勤月由完整周组成，周目标随节假日和调休调整；同月其他周的额外有效出勤可以补足缺口，每月最多使用 3 天跨周补充机会。旧文档中“所有周固定计 5 天”“多出天数不计入”“剩余机会与额外出勤无关”的结论不应继续使用。

周标题改为学校判定合格的实际天数，删除“已满 5 天不计入”的本地推断。月度汇总和合格结果仍以学校返回值为准，不在客户端重建其完整抵扣算法。

验证不保存账号、Cookie 或个人明细；新增测试使用合成统计字段验证来源优先级、缓存重新解析、剩余额度和缺失字段处理。

本次验证：221 个单元测试通过，Release 构建成功，试用包 `dist/slaier-1.1.1-attendance-stats-fix.apk` 的签名与 1.1.1 一致。版本号仍为 1.1.1（5），待用户试用确认后再提交和发布。


## 2026-09-14：跨午夜进出配对与历史时长修复

- 根因：原 `PunchPairing.daily()` 先按自然日期分组，导致午夜前进门和次日凌晨出门无法配对。UI 又优先展示本地累计值，覆盖了学校已经提供的历史时长。
- 修复：先按完整时间顺序配对，再把区间及其出门记录归入进门日期。保留本地未闭合区间的次日 05:00 保护边界，避免与数日后的无关出门记录错误配对。
- 月份查询扩展边界，包含下月第一天的出门记录；首页刷新和今日观察也保留前一天的流水上下文。
- 历史日期直接使用学校累计时长（包括有效的 0 值），不再被不完整的本地计算覆盖；当天未结算时仍可用流水实时估算。
- 本地累计先求秒数之和，再换算成分钟，避免多段记录逐段舍入造成少算。
- 跨天记录显示“次日”出门时间，未闭合的历史记录显示“未匹配出门”，不再同时显示“在馆中”和“作废”。
- 用合成流水先复现失败：3 个定向回归测试中，跨午夜配对和逐段舍入 2 项失败。修复后 235 个单元测试全部通过，Release 构建成功。测试覆盖跨午夜、跨月、重复入场、截止边界、历史学校值优先和今日实时估算。
- 本次没有访问或保存反馈者的账号与原始流水；截图不能确定其具体的凌晨出门时间，仍需安装试用包并刷新验证实际记录。


## 请假日时长显示回归修复

1.1.3 的历史时长优先级没有区分请假日，将学校考勤统计时长直接作为实际在馆时长展示。用户确认新的显示规则：请假日只累计实际进出，非请假日保留现有逻辑，学校合格判定和有效天数不变。

- `leave == true` 时只使用已配对或仍有效的在馆区间；不从学校统计时长减去固定数值，也不回退到学校折算时长。
- 没有可用区间时返回未知并显示“暂无进出记录”；真实的不足一分钟区间仍可显示 0 分钟，不能与未知混同。
- 首页今日卡、考勤今日卡、历史明细及可见的月度时长合计采用相同规则；请假日未配对提示不再声称“累计时长以学校统计为准”。
- 跨午夜配对保留，非请假日的历史学校值优先级保留，`isQual` 和月度有效天数不作本地改写。
- 新增 9 项合成数据回归测试，修改前有 7 项失败；修复后全部 244 项单元测试通过。覆盖带请假折算值的完整流水、缺失流水、孤立出门、过期进门、跨午夜、今日实时估算和非请假兼容性。
