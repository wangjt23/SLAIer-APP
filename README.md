...
- **数据不会丢**：签名一致，属于覆盖安装，登录状态、课表与考勤缓存都保留。

不想要某个版本：点「忽略此版本」，之后只会在出现更高版本时再提示。

**手动更新**（卡住时用，或想自己校验）：

1. 打开 [Releases](../../releases) 下载最新 `slaier-x.y.z.apk` 与 `SHA256SUMS.txt`；
2. 校验：`shasum -a 256 slaier-x.y.z.apk`（与 `SHA256SUMS.txt` 对照）；
3. 手机点开 APK 安装 —— 直接覆盖，**不用卸载**。

> 做不到的事：Android 不允许普通应用静默安装，所以最后一步的系统确认必须由你点。
> App 也不会自动下载：发现新版本后由你决定是否下载安装。
...
学院的判定由**学校服务端**给出，App 只读取、不推断（它只自己算"时长"）：
...
### 教务系统有「三个地址」，但设置里只需要填一个

那一个地址会被自动拆成三个角色 —— 填站点地址或填 `…/yjsxt` 都一样：

| 角色 | 值 | 说明 |
|---|---|---|
| 站点 | `https://sis.slai.edu.cn` | 首页只是个跳板：290 字节静态页，JS 把浏览器送到下面那个入口 |
| 应用根 | `https://sis.slai.edu.cn/yjsxt` | 上面那些接口的前缀，缺了它接口全都不存在 |
| 登录入口 | `https://sis.slai.edu.cn/yjsxt/htxylogin` | `302 → sts.slai.edu.cn` 的 AD FS |

两个坑都实测过：

1. **把站点根当接口前缀** → 登录能过，但请求丢了 `/yjsxt`，课表静默拉不到；
2. **把 `/yjsxt` 当登录入口** → 未登录访问业务路径会被 302 到正方自带的
   `xtgl/login_slogin.html`：那个表单没有任何通往统一身份认证的链接，学生也没有本地密码，
   进去就登不出来。App 遇到这个页面会自动改走 SSO 入口，不会把人留在死胡同里。

设置页会把推导出来的「登录入口 / 课表接口前缀」显示出来，方便对照。
